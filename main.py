...
COMPANY_DESC        = os.environ.get("COMPANY_DESC",         "a company running professional webinars")
COMPANY_DOMAIN      = os.environ.get("COMPANY_DOMAIN",       "yourcompany.com")
...
Distribution: 2 wins, 2 risks, 2 opportunities, 1 action, 1 trend. Tailor every insight to the business context of {COMPANY_NAME}.
...
async def chat(payload: dict, db: Session = Depends(get_db), account_id: str = Depends(get_account_id)):
    ...
    STOPWORDS = {'What','When','Which','Who','How','Why','Where','The','This','That','First','Last','Top','Show','Tell','Give','List','Find','Webinar','Speaker','Attendance','Registration',COMPANY_NAME,COMPANY_NAME.split()[0] if ' ' in COMPANY_NAME else COMPANY_NAME,'WebinarIQ'}
    candidates = []
...
async def ml_analysis(payload: dict, db: Session = Depends(get_db), account_id: str = Depends(get_account_id)):
    ...
    comm_prompts = {
        "email_invite": (
            ...
            "  • Closing: sign off from the team\n"
